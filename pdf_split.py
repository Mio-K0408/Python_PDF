...
start_pages = [1,4,63,121,152,164,184,198,207,221,246,259,309,317]
end_pages = [3,62,120,151,163,183,197,206,220,245,258,308,316,330]
names = ["導入","Javaの特徴","基本構文","クラスの連携","オブジェクト指向とは",\
         "継承","インターフェース","カプセル化","ポリモルフィズム","例外処理",\
         "Javadocの読み方","APIの活用","デバッガの使い方","そのほかの文法"]


for file_name in glob.glob(r'C:\Users\Mio\Desktop\sprid_prac\JavaProgramming.pdf'):    #ファイル名の取得
...
"""
for name,start_page,end_page in zip(names,start_pages,end_pages):
    pdf_file_writer = PdfFileWriter()
    pdf_file_name = name + '.pdf'
    with open(pdf_file_name,'wb') as f:
        for num in range(start_page-1,end_page):
            pdf_file_writer.addPage(pdf_file_reader.getPage(num))
